...
def apply_rules(text: str, rules: List[Rule]) -> Dict:
    ...
    for r in rules:
        for m in re.finditer(r.pattern, text, flags=re.IGNORECASE):
            violations.append({"rule": r.name, "span": [m.start(), m.end()], "match": m.group(0), "message": r.message, "action": r.action})
            if r.action == "redact":
                redacted = redacted.replace(m.group(0), "[REDACTED]")
            if r.action == "block":
                # keep redacted as-is; caller can decide
                pass
    return {"ok": not any(v["action"]=="block" for v in violations), "text": redacted, "violations": violations}
...
def default_rules() -> List[Rule]:
    return [
        ...
        Rule("prompt_injection", r"(?s)ignore previous|override instructions|system prompt", "Prompt-injection cue", action="block"),
    ]
